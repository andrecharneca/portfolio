...
export default function ProfileSection({ profile }: ProfileProps) {
  ...
  return (
    <section
      ref={containerRef}
      className="p-4 min-h-[30vh] h-screen flex items-center justify-center pt-24 ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0'}"
    >
      <div className="flex flex-col sm:flex-row items-center gap-6">
        ...
        <div>
          <h2
            ref={nameRef}
            className="py-4 text-2xl md:text-4xl sm:text-xs font-semibold"
          >
            {profile.name}
          </h2>
          <p ref={positionRef} className="text-xl md:text-xl">
            {profile.position}
          </p>
          <p ref={locationRef} className="text-lg md:text-xl mt-1">
            {profile.location}
          </p>
